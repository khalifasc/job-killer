...
class Job_Killer_Helper {
    ...
    public function get_import_stats() {
        ...
        $today_imports = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $imports_table WHERE DATE(imported_at) = %s",
            current_time('Y-m-d')
        ));
        
        // Imports this week
        $week_start = date('Y-m-d', strtotime('monday this week'));
        $week_imports = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $imports_table WHERE imported_at >= %s",
            $week_start
        ));
        
        // Imports this month
        $month_start = date('Y-m-01');
        $month_imports = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $imports_table WHERE imported_at >= %s",
            $month_start
        ));
    ...
    public function get_chart_data($days = 30) {
        ...
        $results = $wpdb->get_results($wpdb->prepare("
            ...
            FROM $imports_table 
            WHERE imported_at >= DATE_SUB(NOW(), INTERVAL %d DAY)
