...
class GoFetch_Importer {
	...
	public function __construct() {
		...
		add_filter( 'goft_wpjm_item_title', array( __CLASS__, 'prettify_title' ), 10, 2);

		add_filter( 'goft_wpjm_sample_item', array( $this, 'sample_item' ), 20, 2 );

		add_filter ( 'goft_wpjm_valid_item', array( $this, 'validate_item' ), 10, 3 );

		add_filter( 'goft_wpjm_fetch_feed_item', array( __CLASS__, 'set_item_core_atts' ), 10, 3 );
		add_filter( 'goft_wpjm_fetch_feed_item', array( __CLASS__, 'merge_unknown_items' ), 11, 5 );

		add_filter( 'goft_wpjm_fetch_feed_sample_item', array( __CLASS__, 'unset_sample_item_core_atts' ), 11, 3 );

		add_filter( 'goft_wpjm_processed_feed_item', array( $this, 'processed_feed_item' ), 10, 2 );
		add_filter( 'goft_wpjm_import_item_apply_regexp', array( $this, 'item_apply_regexp' ), 10, 3 );

		add_filter( 'goft_wpjm_skip_loading_as_feed', array( $this, 'skip_loading_as_feed_if_provider_limits_requests' ), 10, 2 );

		add_filter( 'goft_wpjm_content_blocked', array( $this, 'provider_blocked_warning' ), 10, 3 );
	 ...
	 */
	public static function import_feed( $orig_url, $params = array(), $cache = true, $is_file_upload = false ) {
		global $goft_wpjm_options;
		...
		for ( $i = 1; $i <= $pages; $i++ ) {
			...
			if ( $pages > 1 ) {
				...
				if ( ! apply_filters( 'goft_wpjm_use_custom_feed_pagination', false, $provider_id, $url, $page_qarg_val, $pages ) ) {
					$url = add_query_arg( array( $page_qarg => $page_qarg_val, $limit_qarg => $max_results ), $url );
				} else {
					$url = apply_filters( 'goft_wpjm_custom_feed_pagination', $url, $provider_id, $page_qarg_val, $pages );
				}
			...
			if ( is_wp_error( $results_temp ) && 1 === $i ) {
	...
	public static function content_is_cf_blocked( $html, $xpath = false ) {
		...
		$tags = $xpath->query( '//*[contains(text(),"Checking if the site connection is secure") or contains(text(),"Access denied") or contains(text(),"error code") or contains(text(),"Enable JavaScript and cookies to continue") or contains(text(),"you have been blocked")]' );
		return $tags->length;
	}

	/**
	 * Identify XML files that use a <Field name=""> structure.
	 */
	protected static function uses_field_tag_names( $xml ) {
		if ( ! empty( $xml->job ) ) {
			$root = $xml->job;
		} else if ( ! empty( $xml->item ) ) {
			$root = $xml->item;
		} else if ( ! empty( $xml->channel->item ) ) {
	...
	/**
	 * Convert XML files that use a <Field name=""> structure to a regular list of fields.
	 */
	protected static function get_items_from_field_tag_names( $xml ) {
		$items = array();
		foreach( $xml->job as $job ) {
			$flat_job = (array) $job;
			$index = 0;
			foreach( $job->Field as $Field ) {
				$value = $flat_job['Field'][ $index ];
				foreach( (array) $Field->attributes() as $a => $b ) {
					$item[ $b['name'] ] = $value;
				}
				$index++;
			}
	...
	public static function import_custom_content_type( $url_or_data, $is_file_upload = false, $retries = 0 ) {
		...
		if ( self::content_is_cf_blocked( $response ) ) {
			if ( $retries <= apply_filters( 'goft_wpjm_importer_content_cf_unblock_max_retries', 5 ) ) {
				...
				return apply_filters( 'goft_wpjm_content_blocked', new WP_Error( -1005, __( 'Content is blocked by Cloudflare.', 'gofetch-wpjm' ) ), $valid_provider, $url_or_data );
			}
		}
		...
		if ( GoFetch_Helper::is_json( $response ) ) {
			...
			if ( ! is_array( $response ) ) {
				...
				if ( ! is_array( $response ) ) {
					return apply_filters( 'goft_wpjm_json_content_not_readable_error', new WP_Error( -1002, __( 'JSON Content is not readable.', 'gofetch-wpjm' ) ), $valid_provider, $url_or_data );
				}
			}
			...
			if ( ! $xml ) {
				return apply_filters( 'goft_wpjm_xml_content_not_readable_error', new WP_Error( -1003, $response ), $valid_provider, $url_or_data );
			}
			...
			if ( self::uses_field_tag_names( $xml ) ) {
				$items = self::get_items_from_field_tag_names( $xml );
			} else {
				//
	...
	protected static function _import_feed( $url, $provider_id, &$provider = '' ) {
		...
		if ( $provider_id && GoFetch_Helper::is_api_provider( $provider_id, $provider ) ) {
			...
			if ( is_wp_error( $feed ) || GoFetch_Helper::is_gziped( $url ) ) {
				...
				if ( ! is_a( $feed, 'SimplePie' ) || GoFetch_Helper::is_gziped( $url ) ) {
					$feed = apply_filters( 'goft_wpjm_import_custom_content_type', self::import_custom_content_type( $url ), $url );
				}

				// @todo: MAYBE CUT BIG FILES TO SMALLER ITEMS
		...
		$results = call_user_func( $import_callback, $feed, $url, $provider, $custom_content_type );
	 ...
	 */
	public static function fetch_feed_items( $feed, $url, $provider, $custom_content_type = false ) {
		global $goft_wpjm_options;
		...
		foreach ( (array) $items as $item ) {
			...
			$sample_item = apply_filters( 'goft_wpjm_fetch_feed_sample_item', $sample_item, $provider, $item );
	...
	public static function process_feed_item( $item, $namespaces, $provider ) {
		...
		if ( gfjwjm_fs()->is__premium_only() ) {

			// Look for valid custom item tags on each of the feed namespaces.
			foreach ( $namespaces as $attribute => $namespace ) {

				if ( empty( $item->data['child'][ $namespace ] ) ) {
					continue;
				}

				// @todo: move to function
				foreach ( $item->data['child'][ $namespace ] as $tag => $sub_tags ) {
					...
					foreach ( (array) $curr_tag as $key => $c_tag ) {
						...
						$new_data = $item->data['child'][ $namespace ][ $c_tag ][ $key ]['data'];

						if ( $new_data ) {
							$data .= $new_data;
						} else {
							if ( ! empty( $item->data['child'][ $namespace ][ $c_tag ][ $key ]['child'][ $namespace ] ) ) {
								// @todo: move to function
								foreach ( $item->data['child'][ $namespace ][ $c_tag ][ $key ]['child'][ $namespace ] as $_key => $_sub_tags ) {
									// Iterate through sub-tags until we have a value
									foreach( $_sub_tags as $__key => $_sub_tag ) {
										$new_data = $_sub_tag['data'];
										if ( $new_data ) {
											$data .= $new_data;
											break;
										}
						...
						// Skip filter for 'description' tag.
						if ( 'description' !== $curr_tag && 'encoded' !== $curr_tag ) {
							$data = apply_filters( 'goft_wpjm_import_items_namespace_data', html_entity_decode( wp_strip_all_tags( $data ) ), $item, $curr_tag, $namespace, $data );
						}
					...
					if ( isset( $data ) ) {
						...
						// Maybe log import info.
						$vars = array(
							'context'  => 'GOFT :: IMPORTING ITEM FROM RSS FEED (VALID NAMESPACES)',
							'item'     => $new_item['title'],
							'tag'      => $tag,
							'data'     => $data,
						);
			...
			if ( ! empty( $new_item['geolocation'] ) ) {
				...
				if ( ! empty( $geolocation[0] ) && ! empty( $geolocation[1] ) ) {
					$new_item['latitude']  = trim( $geolocation[0] );
					$new_item['longitude'] = trim( $geolocation[1] );
				}
			}
		}
	...
	public static function import( $items, $params ) {
		...
		if ( empty( $items ) ) {
			...
			$results = self::filter_items( $items, array( 'post_type' => $params['post_type'] ), $params );
			...
			// Restore. Deferred database process are commited at this time.
			$wpdb->query( 'COMMIT;' );
	...
	private static function map_item_param_fields( $params, $item ) {
		...
		foreach ( $params['field_mappings'] as $item_field => $custom_field ) {
			...
			if ( ! isset( $item[ $item_field ] ) || is_array( $item[ $item_field ] ) || strpos( $item[ $item_field ], '_reserved_' ) !== false ) {
				continue;
			}
	...
	public static function _import( $item, $params ) {
		...
		if ( ! apply_filters( 'goft_wpjm_valid_item', true, $item, $params ) ) {
			return false;
		}
		...
		if ( ! is_wp_error( $post_data ) ) {
			...
			$meta = self::_prepare_item_meta( $item, $original_item, $params['meta'], $post_data, $params );
			...
			if ( ! is_array( $post_data ) && is_numeric( $post_data ) ) {
				...
				do_action( 'goft_wpjm_after_insert_job', $post_data, $item, $params, $meta );
				// return the post_id.
				return $post_data;
			}
		}
		...
		if ( is_wp_error( $post_data ) ) {
			...
			do_action( 'goft_wpjm_insert_job_failed', $item, $params, $meta );
			return $post_data;
		} else {
			return apply_filters( 'goft_wpjm_custom_job_insert', false, $post_data, $item, $params, $meta );
		}
		return false;
	}
	...
	private static function _insert_post( $item, $params ) {
		...
		$post_content = apply_filters( 'goft_wpjm_post_content', wp_kses_post( $post_content ), $item, $params );

		$post_title = ! empty( $params['post']['post_title'] ) ? $params['post']['post_title'] : $item['title'];
		$post_title = apply_filters( 'goft_wpjm_post_title', wp_kses_post( $post_title ), $item, $params );
		...
		if ( ! empty( $params['field_mappings'] ) && ( $date_key = array_search( 'date', $params['field_mappings'] ) ) ) {
			...
			$post_arr['post_date'] = apply_filters( 'goft_wpjm_post_date', wp_kses_post( $post_date ), $item, $params );
		}
		...
		if ( ! empty( $params['smart_tax_input'] ) ) {
			...
			if ( 'title' === $goft_wpjm_options->keyword_matching ) {
				$content = $post_title;
			} else if ( 'content' === $goft_wpjm_options->keyword_matching ) {
			...
			if ( method_exists( 'GoFetch_Premium_Starter_More_Features', 'smart_tax_terms_input' ) ) {
				$params['tax_input'] = GoFetch_Premium_Starter_More_Features::smart_tax_terms_input( $params['tax_input'], $item, $content, self::$used_taxonomies, $params['smart_tax_input'] );
			}
		}

		$params['tax_input'] = apply_filters( 'goft_wpjm_post_tax_input', $params['tax_input'], $item, $params  );
		...
		if ( apply_filters( 'goft_wpjm_skip_import_job', false, $item, $params, $post_arr ) ) {
			return $post_arr;
		}
		...
		do_action( 'goft_wpjm_before_job_insert', $post_arr, $item, $params );

		$post_arr = apply_filters( 'goft_wpjm_post_insert', $post_arr, $item, $params );
	...
	private static function _add_meta( $post_id, $meta ) {
		...
		foreach ( $meta as $meta_key => $meta_value ) {
			...
			if ( apply_filters( 'goft_wpjm_update_meta', false, $meta_key, $meta_value, $post_id ) ) {
				continue;
			}
	 ...
	 */
	private static function _prepare_item_meta( $item, $original_item, $meta, $post_id, $params ) {
		...
		foreach ( $meta as $meta_key => $meta_value ) {
			 ...
			 */
			if ( ! apply_filters( 'goft_wpjm_item_skip_meta', false, $meta_value, $meta_key, $item, $post_id, $params ) ) {
				$final_meta[ $meta_key ] = apply_filters( 'goft_wpjm_item_meta_value', $meta_value, $meta_key, $item, $post_id, $params );
			}
		}
	...
	public static function standardize_item_keys( $item, $field_mappings ) {
		...
		if ( ! empty( $field_mappings ) && array_intersect( array_values( $field_mappings ), array_keys( $base_alt_fields ) ) ) {
	...
	private static function filter_items( $items, $args = array(), $params = array() ) {
		...
		$keywords_filtering          = $params['keywords_filtering'];
		...
		try {
			...
			// Get the item title based on the user mappings
			$title_field = GoFetch_Admin_Builder::get_core_mapped_field( 'post_title', $params['field_mappings'] );

			if ( ! $title_field ) {
				$title_field = 'title';
			}

			$all_items_titles = wp_list_pluck( $items, $title_field );

			$like_titles = '';

			foreach ( $all_items_titles as $all_items_title ) {
				if ( empty ( $all_items_title ) ) continue;
				$all_items_title = str_replace( '&#039;', '\&#039;', $all_items_title );
				if ( $like_titles ) $like_titles .= ' OR ';
				$like_titles .= ' TRIM( LOWER( c.meta_value ) ) LIKE "%%' . addslashes( strtolower( trim( $all_items_title, ENT_QUOTES ) ) ) . '%%"';
			}

			if ( ! empty( $like_titles ) && strpos( strtolower( $like_titles ), ' like ' ) >= 0 ) {
			...
			if ( ! empty( $results ) ) {
				...
				foreach ( $results as $post ) {
					...
					$external_posts[] = array(
						'jobkey'      => $jobkey,
						'post_id'     => $post->ID,
						'title'       => GoFetch_Helper::clean_and_sanitize( $title, $_alphanumeric_only = true ),            // Remove all whitespace (including tabs and line ends).
						'description' => GoFetch_Helper::clean_and_sanitize( $description, $_alphanumeric_only = true ), // Remove all whitespace (including tabs and line ends).
						'from_db'     => true,
					);
		...
		if ( $keywords_exclude ) {
			...
			$keywords_exclude = array_filter( $keywords_exclude );
		}
		...
		foreach ( $items as &$item ) {
			...
			if ( ! $dup_post_id ) {
				...
				// Match keywords.
				if ( ( $keywords_filtering && ( $keywords || $keywords_exclude ) ) && $content ) {
					...
					$exclude = apply_filters( 'goft_wpjm_exclude_item', $exclude, $item, $keywords, $keywords_exclude, $keywords_comparison, $keywords_exclude_comparison );
	...
	public static function fetch_feed( $url, $provider ) {
		...
		if ( is_wp_error( $feed ) ) {
			...
			if ( apply_filters( 'goft_wpjm_fetch_feed_simplexml', false !== stripos( $feed->get_error_message(), 'feed could not be found at' ) ) ) {
				$context = stream_context_create( array( 'http' => array( 'header' => 'Accept: application/xml', 'ignore_errors' => true ) ) );

				$xml = @file_get_contents( $url, false, $context );
				$feed = self::fetch_feed_raw_data( $xml, $url );

				$valid_feed = ! is_wp_error( $feed );
				...
				$context  = stream_context_create( array( 'http' => array( 'header' => 'Accept: application/xml\r\nOrigin: ' . home_url(), 'ignore_errors' => true ) ) );
		...
		remove_action( 'wp_feed_options', array( __CLASS__, 'set_feed_options' ), 10, 2 );
	...
	public function processed_feed_item( $item, $provider ) {
		...
		if ( apply_filters( 'goft_wpjm_import_apply_regexp', ! empty( $provider['feed']['regexp_mappings'] ), $provider, $item ) ) {
			$item = apply_filters( 'goft_wpjm_import_item_apply_regexp', $item, $provider, $item );
		}
		...
		if ( ! empty( $provider['feed']['regexp_mappings'] ) ) {
			...
			foreach ( $valid_regexp_tags as $key => $patterns ) {
				...
				foreach ( $patterns as $pattern ) {
					$parts = self::get_item_regex_parts( $description, array( $key => $pattern ) );
					if ( $parts ) {
						break;
					}
				}
	...
	public function sample_item( $item, $provider ) {
		$item['provider_id'] = null; unset( $item['provider_id'] );
		return $item;
	}

	/**
	 * Validate an item before importing it.
	 */
	public function validate_item( $valid, $item, $params ) {
	 ...
	 */
	public static function merge_unknown_items( $item, $provider, $url, $original_item = array(), $defaults = array() ) {
		if ( empty( $original_item ) ) {
			return $item;
		}
	...
	public static function unset_sample_item_core_atts( $sample_item, $provider, $orig_item ) {
		...
		if ( isset( $sample_item['logo'] ) ) {
			$sample_item['logo'] = sprintf( '<img src="%s" style="max-height: 32px; max-width: 100%%;" />', esc_url( str_replace('%', '%%', $sample_item['logo'] ) ) );
		}
